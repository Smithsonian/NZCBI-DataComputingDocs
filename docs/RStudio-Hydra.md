...
4. The "Files" tab shows Hydra's storage systems. You have access to Hydra's `/home`, `/scratch`, `/data`, and `/store` directories. You can use the [R-Studio](https://posit.co/download/rstudio-desktop) Server interface to transfer files or use other file transfer tools.  Click on the "..." to change to the location where your files are located.
...
### b) [R](https://cran.r-project.org/) session
...
For additional information on using [R-Studio](https://posit.co/download/rstudio-desktop) Server, please see the full reference documentation provided on the High Performance Computing website: [Using the R-Studio Server](https://confluence.si.edu/spaces/HPC/pages/385975502/Using+the+RStudio+Server).